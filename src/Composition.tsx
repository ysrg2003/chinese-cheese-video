...
function StoryboardVisuals({ job, second }: { job: VideoJob; second: number }) {
  ...
  return <>
    ...
    {(kind === "board_overview" || kind === "army_setup" || kind === "board_identity") && <>
      ...
      {kind === "board_identity" && <><Marker left={board.x + 95} top={board.y + 88} opacity={opacity} tone="gold">9 FILES</Marker><Marker left={board.x + board.width - 94} top={board.y + board.height - 88} opacity={opacity} tone="gold">10 RANKS</Marker><div style={{ position: "absolute", left: board.x + 28, top: board.y + 28 + 4 * cell, width: grid.width, height: cell, background: "rgba(74,154,194,.28)", borderTop: "4px solid rgba(189,231,255,.9)", borderBottom: "4px solid rgba(189,231,255,.9)", opacity, zIndex: 3 }} /></>}
    </>}

    {kind === "two_armies" && <><div style={{ position: "absolute", left: board.x + 28, top: board.y + 28, width: grid.width, height: grid.height / 2 - 10, background: "linear-gradient(180deg, rgba(35,35,35,.46), rgba(35,35,35,.06))", opacity, zIndex: 3 }} /><div style={{ position: "absolute", left: board.x + 28, top: board.y + 28 + grid.height / 2 + 10, width: grid.width, height: grid.height / 2 - 10, background: "linear-gradient(0deg, rgba(182,60,47,.42), rgba(182,60,47,.06))", opacity, zIndex: 3 }} /><Marker left={board.x + board.width / 2} top={board.y + 250} opacity={opacity} tone="black">BLACK ARMY ↓</Marker><Marker left={board.x + board.width / 2} top={board.y + board.height - 250} opacity={opacity} tone="red">↑ RED ARMY</Marker></>}
    ...
    {kind === "river_palaces" && <><div style={{ position: "absolute", left: board.x + 28, top: board.y + 28 + 4 * cell, width: grid.width, height: cell, background: "rgba(60,145,194,.54)", borderTop: "4px solid #b9eaff", borderBottom: "4px solid #b9eaff", opacity, zIndex: 3 }} /><div style={{ position: "absolute", left: board.x + 28 + 3 * cell, top: board.y + 28, width: 2 * cell, height: 2 * cell, border: "7px solid #f5ce74", borderRadius: 12, opacity, zIndex: 4 }} /><div style={{ position: "absolute", left: board.x + 28 + 3 * cell, top: board.y + 28 + 7 * cell, width: 2 * cell, height: 2 * cell, border: "7px solid #f5ce74", borderRadius: 12, opacity, zIndex: 4 }} /><Marker left={board.x + board.width / 2} top={board.y + 28 + 4.5 * cell} opacity={opacity} tone="blue">THE RIVER</Marker><Marker left={board.x + board.width / 2} top={board.y + 28 + 2.25 * cell} opacity={opacity}>BLACK PALACE</Marker><Marker left={board.x + board.width / 2} top={board.y + 28 + 6.75 * cell} opacity={opacity} tone="red">RED PALACE</Marker></>}
    ...
    {kind === "coordinate_map" && <><div style={{ position: "absolute", left: board.x + 28, top: board.y - 18, width: grid.width, display: "flex", justifyContent: "space-between", opacity, zIndex: 5 }}>{Array.from({ length: 9 }).map((_, index) => <Marker key={index} left={index * cell} top={0} opacity={opacity}>F{index + 1}</Marker>)}</div><div style={{ position: "absolute", left: board.x - 18, top: board.y + 28, height: grid.height, display: "flex", flexDirection: "column", justifyContent: "space-between", opacity, zIndex: 5 }}>{Array.from({ length: 10 }).map((_, index) => <Marker key={index} left={0} top={index * cell} opacity={opacity}>R{index + 1}</Marker>)}</div><div style={{ position: "absolute", left: boardPoint(4, 4).x - 42, top: boardPoint(4, 4).y - 42, width: 84, height: 84, borderRadius: 999, border: "7px solid #f5ce74", opacity, zIndex: 5 }} /></>}
