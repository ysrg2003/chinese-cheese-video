...
يستخدم النظام Remotion للرندر، وAI Provider Router لتوليد التعليق الصوتي عبر Gemini-TTS بصوت ذكوري، مع Edge-TTS كمسار legacy اختياري، ومخرجاً اختيارياً عبر Gemini أو Ollama، وقاعدة بيانات SQLite محلية تعمل مباشرة دون Supabase. يبقى Supabase مساراً اختيارياً فقط، ويعود النظام تلقائياً إلى SQLite إذا كان Supabase غير متاح أو غير مهيأ.
...
| مسار الصوت القديم | `edge_tts` عند طلبه صراحة فقط |
...
| Edge-TTS legacy | مسار احتياطي اختياري عند ضبط `TTS_PROVIDER=edge_tts` صراحة | `python/tts.py` |
