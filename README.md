...
يستخدم النظام Remotion للرندر، وAI Provider Router لتوليد التعليق الصوتي عبر Gemini-TTS بصوت ذكوري، ثم يستخدم Edge-TTS كـfallback أخير فقط بعد استنفاد جميع مسارات AI Router، ومخرجاً اختيارياً عبر Gemini أو Ollama، وقاعدة بيانات SQLite محلية تعمل مباشرة دون Supabase. يبقى Supabase مساراً اختيارياً فقط، ويعود النظام تلقائياً إلى SQLite إذا كان Supabase غير متاح أو غير مهيأ.
...
| مزود الصوت الإنتاجي | `ai_router` عبر AI Provider Router |
| صوت الإنجليزية الرجالي | `Charon` — Gemini-TTS |
| صوت الصينية الرجالي | `Charon` — Gemini-TTS |
| fallback الصوت الأخير | `edge_tts` بعد فشل جميع مسارات AI Router، أو عند طلبه صراحة |
| مفتاح تفعيل fallback | `TTS_EDGE_FALLBACK_ENABLED=1` افتراضياً |
...
| AI Provider Router TTS | توليد الصوت الذكوري وتحويل PCM إلى MP3 مع توقيت يعتمد على مدة الصوت | `python/tts.py`, `python/ai_router_bridge.py` |
| Edge-TTS last resort | fallback أخير بعد terminal failure من AI Router، مع تسجيل `edge_tts_last_resort` | `python/tts.py` |
