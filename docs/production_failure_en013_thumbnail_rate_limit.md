...
A normal production run records this condition as a `deferred` item rather than a failed candidate. The curriculum episode remains retryable, no stale public state is relabelled as a content failure, and the workflow exits successfully after its reconciliation attempt. This prevents a temporary YouTube thumbnail quota from producing false red production runs while still holding the next prerequisite-dependent lesson until the channel policy is complete.

When `reconcile_youtube.py` finally returns `published`, it now updates the matching curriculum episode in the same transaction path using the persisted `curriculum_lesson_key`. The next normal scheduled production run can therefore select en-014 immediately; it does not spend a whole extra cycle merely re-marking the already-public en-013 episode as published.
