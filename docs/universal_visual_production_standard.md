...
## Optional reference-edited visual assets

A second, independent stage may add **zero to two reference-edited assets** to a job. `visual_assets.py` sends the completed storyboard and spoken context to the AI Router as a constrained **asset planner**. The planner may select only a non-move scene with a deterministic editable region, currently the river, palace, or rule-focus overlays. It returns a short JSON plan with a scene index, approved role, and a localized English `editPrompt`. It is not allowed to request a new board, a new scene, or a full-image generation.

For each selected scene, the pipeline first renders the **actual Remotion scene** as a 1080×1920 PNG. It then creates an aligned transparent mask: opaque pixels protect the original frame and transparent pixels expose only the exact region that may change. For example, the river lesson uploads the complete original Xiangqi board and opens only the existing rectangular river band. The image service receives the reference, mask, and editPrompt through `/v1/visual-assets/reference-edits`; it does not receive a free-standing request such as “generate a river.”

The service returns an edited image, after which a deterministic compositor restores every pixel outside the mask from the original reference. The validated PNG is saved under `public/generated/<job-id>/assets/`, while the reference and mask remain in the private job artifact for inspection. Remotion uses the edited result only as a short establishing layer; the canonical board, pieces, moves, captions, and exact overlays remain the teaching authority.
...
| A reference is always the source of truth | The pipeline uploads a real Remotion scene and a same-size mask; it never uses a generated image as the board reference. |
| Exact geometry remains deterministic | Move scenes are ineligible; the edit mask is restricted to non-move regions and the compositor restores all unmasked pixels. |
...
The core visual-standard implementation began in GitHub commit `4a011f0`; the optional asset layer extends it with `visual_assets.py`, protected service credentials, image validation, and Remotion integration. The local suite now passes 33 tests, including generic move storyboard creation, fallback behavior, asset-plan filtering, image-byte validation, generated-asset move protection, asset-to-job attachment, storyboard validation, audio-duration boundaries, fast move animation, YouTube idempotency, playlist association, and curriculum tests. Python compilation and TypeScript typecheck pass. A live visual-assets smoke test previously returned a portrait PNG, proving the legacy optional path. The reference-edit implementation additionally passed a local 1080×1920 Remotion reference and mask inspection: the original board remained intact and only the river rectangle was transparent. The live Space exposes the reference-edit endpoints on commit `99142db`; the next authenticated smoke test must validate the browser upload path before enabling it for a new published video.
