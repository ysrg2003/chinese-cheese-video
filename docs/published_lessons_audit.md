...
The guarded GitHub workflow [31761045747](https://github.com/ysrg2003/chinese-cheese-video/actions/runs/31761045747) authenticated against channel `UCM7pTdgZRwDZ2gZDtC6SITg`, completed its preflight, and finished successfully. The YouTube API reported seven target IDs as already absent and permanently deleted the remaining public video `QEdAG1azW2U`. The seven already-absent IDs were `wmJ5-34N6z8`, `KkaGX4ujyfI`, `M7mQrRxIg-M`, `na82AsZBxKU`, `6uZ1lxn-oUs`, `tA3vZMgrfg8`, and `gSgVXtG9Snw`. The local quarantine updated all eight historical publication rows and normalized catalog rows to `deleted_invalid_content`, cleared their active video and playlist identifiers while preserving the historical IDs in remediation metadata, reset four reusable candidates to `discovered`, and reset `en-010-the-general` to `retry`. The replacement subsequently converted only the en-010 row to the new validated public video while the other seven remain quarantined. The three non-curriculum evergreen candidates (`evergreen-33-0`, `evergreen-33-1`, and `evergreen-33-7`) are now `retired_invalid_content`, so the autonomous selector cannot regenerate the same historical topics as duplicates. The two deleted standalone Chinese news rows are also intentionally not regenerated as duplicate uploads; their language requirement is covered by the Chinese audio, caption, and localized metadata artifacts attached to the corrected English-primary publication.

The six static curriculum lessons should remain public. They contain no stored moves and are not implicated by this move-legality audit. The final active catalog therefore contains six safe static lessons, one corrected move-bearing en-010 replacement, and seven historical deleted-invalid rows retained only for remediation evidence.
